...
public class ReceiptDAOImpl implements ReceiptDAO{
    ...
    public void saveDelivery(Delivery delivery, String login){
        jdbcTemplate.update("INSERT INTO delivery (Receipt_idReceipt, delivery_info, name)\n" +
    ...
    public List<Order> index(String login) {
        return jdbcTemplate.query("select idReceipt, receipt.price, status.status as receipt_status, user_iduser"+
                ...
                " where user_iduser = (select idUser from user where login = ?)" +
                " order by idReceipt DESC", new Object[]{login}, new ReceiptMapper());
    }
    ...
    public Delivery showDeliveryInfo(int idReceipt) {
        return jdbcTemplate.query("select idDelivery, name, delivery_info from delivery where Receipt_idReceipt = ?",
    ...
    public List<Product_basket> index_product_basket(int receipt_id) {
        return jdbcTemplate.query("select product.idProduct, product.name, type.name as type, \n"+
                ...
                "where idReceipt = ?", new Object[]{receipt_id}, new Product_basketMapper());
    }
    ...
    public void changeStatus(String status, int idReceipt) {
        ...
        jdbcTemplate.update("update receipt set idStatus = ? where idReceipt = ?", idStatus, idReceipt);
    }
}
